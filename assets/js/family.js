                  ...
                  ${product.sizes
                    .map(
                      ...
                      <div class="rounded-3xl border border-gray-200 bg-gray-50 p-4 shadow-sm">
                        ...
                        <button onclick="sendOrder('${product.name.replace(/'/g, "\\'")}', '${size.label.replace(/'/g, "\\'")}', '${size.concentration ? size.concentration.replace(/'/g, "\\'") : ""}', '${size.cartons ? size.cartons.replace(/'/g, "\\'") : ""}')" class="mt-3 w-full bg-[#cc0000] text-white py-2 rounded-full font-bold hover:bg-red-800 transition shadow-sm text-xs" title="طلب ${product.name} - ${size.label}" aria-label="طلب ${product.name} - ${size.label}">
                          اطلب هذا الحجم
                        </button>
                      </div>
                    `,
